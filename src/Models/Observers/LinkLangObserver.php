...
namespace WalkerChiu\MorphLink\Models\Observers;

class LinkLangObserver
{
    /**
    ...
    {
        $query =
            config('wk-core.class.morph-link.linkLang')
                ::withTrashed()
        ...
        if (
            config('wk-morph-link.soft_delete')
            && (
                config('wk-core.lang_log')
                || config('wk-morph-link.lang_log')
            )
        ) {
